var flowy = function (canvas, grab, release, snapping, deleted, click_canvas, spacing_x, spacing_y, vueapp) {
    // ----------[rolence add suport vueapp arg]----------
    ...
    flowy.load = function () {
        ...
        flowy.beginDrag = function (event) {
            // console.log("----------[beginDrag]----------");console.log(blocks[1].y)
        ...
        flowy.endDrag = function (event) {
            if (event.which != 3 && (active || rearrange)) {
                ...
                if (parseInt(drag.querySelector(".blockid").value) === 0 && rearrange) {
                    ...
                    blockSnap(drag, true, undefined, false);
                    active = false;
                    ...
                    for (var i = 0; i < blocks.length; i++) {
                        if (xpos >= blocks.filter(a => a.id == blocko[i])[0].x - (blocks.filter(a => a.id == blocko[i])[0].width / 2) - paddingx
                            ...
                            && ypos >= blocks.filter(a => a.id == blocko[i])[0].y - (blocks.filter(a => a.id == blocko[i])[0].height / 2) - (2 * paddingx)
                            && ypos <= blocks.filter(a => a.id == blocko[i])[0].y + blocks.filter(a => a.id == blocko[i])[0].height + (2 * paddingx)) {
                            active = false;

                            if (blockSnap(drag, false, blocks.filter(id => id.id == blocko[i])[0], rearrange)) {
                                snap(drag, i, blocko);
                            } else {
                                drag.remove();
                                rearrange = false;
        ...
        function touchblock(event) {
            ...
            if (hasParentClass(event.target, "block")) {
                ...
                if (event.type !== "mouseup" && hasParentClass(event.target, "block")) {
                    ...
                    if (event.which != 3 && !active && !rearrange) {
                        if (clickup.target === clickdown.target){
                            // 嘗試加上刪除(失敗)
                            // var child_list = []; // 下游 blockid
                            // (function loopChilds(idval) {
                            //     var idx = blocks.map(x=>x.id).indexOf(idval)
                            //     if (idx != -1) {
                            //         document.querySelector(".arrowid[value='" + idx + "']").parentNode.remove();
                            //         document.querySelector(".blockid[value='" + idx + "']").parentNode.remove();
                            //         blocks.splice(idx, 1)
                            //     }
                            //     child_list.push(idval)
                            //     blocks.filter(a => a.parent == idval).forEach(d => {
                            //         loopChilds(d.id)
                            //     })
                            // })(parseInt(drag.querySelector(".blockid").value))
                            // dragblock = true;
                            // flowy.moveBlock(event)
                ...
                while (!flag) {
                    for (var i = 0; i < layer.length; i++) {
                        if (layer[i] != blockid) {
                            ...
                            // ----------[rolence 修改首顆位移的bug]----------
                            blockParent.style.left = (blockParent.getBoundingClientRect().left + window.scrollX) - (drag.getBoundingClientRect().left + window.scrollX) - 2;
                            blockParent.style.top = (blockParent.getBoundingClientRect().top + window.scrollY) - (drag.getBoundingClientRect().top + window.scrollY) - 2;
                            arrowParent.style.left = (arrowParent.getBoundingClientRect().left + window.scrollX) - (drag.getBoundingClientRect().left + window.scrollX) - 2;
                            arrowParent.style.top = (arrowParent.getBoundingClientRect().top + window.scrollY) - (drag.getBoundingClientRect().top + window.scrollY) - 2;
        ...
        function fixOffset() {
            if (offsetleftold < (canvas_div.getBoundingClientRect().left + window.scrollX)) {
                ...
                for (var w = 0; w < blocks.length; w++) {
                    ...
                    blocks.filter(a => a.id == blocko[w])[0].x = (document.querySelector(".blockid[value='" + blocks.filter(a => a.id == blocko[w])[0].id + "']").parentNode.getBoundingClientRect().left + window.scrollX) + canvas_div.scrollLeft + (blocks.filter(a => a.id == blocko[w])[0].width / 2);
    ...
    function blockReleased(drag) {
        release(drag);
    }

    function blockSnap(drag, first, parent, rearrange) {
        return snapping(drag, first, parent, rearrange);
    }

    function blockDelete(drag, removelist){
        deleted(drag, removelist);
    }

    function blockClick(drag) {
        click_canvas(drag);
    }
