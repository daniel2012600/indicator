...
Vue.component("v-echarts-mixlinebar", {
    props: {
        data: Array,
        dimkey: {
            type: String,
    ...
    methods: {
        ...
        //取得y軸的設定
        get_ylabel_by_data(){
            var yAxis = this.y_label_key.map((ykey,idx) => {
        ...
        //取得series的設定
        get_series(){
            var series = _.map(this.y_label_key, (ylk,idx) =>{
                return this.meskeys.map(ykey => {
                    return {
                        name: ykey,
                        type: this.meskeys_types[idx],
                        data: _.map(_.groupBy(this.data, "name")[ykey], ylk),
                        yAxisIndex: this.meskeys_types.indexOf(this.meskeys_types[idx])
                    };
                });
            })
            return _.flatMap(series)
        },
        render() {
            ...
            var origin_date = _.union(this.data.map(d => {
                if (this.dt_interval != undefined) {
                    var date = this.get＿xlable＿by＿type(d[this.dimkey], this.dt_interval)
                    return date;
                } else {
            ...
            var opt_colors = _.map(this.data, (item, idx) => {
                var sel_color = this.color[item.pay_type];
                if (sel_color) {
                    return sel_color;
                } else {
                    return Object.values(this.color)[idx]
                }
            });
            // 指定图表的配置项和数据
            var option = {
                tooltip: {
                    ...
                    formatter: params => {
                        var res = [];
                        var index = 0
                        _.forEach(_.groupBy(params, "seriesName"), item => {
                            _.forEach(item, subitem => {
                                var value = this.y_label_format(this.fKNum(subitem.data), subitem.seriesName);
                                if(subitem.seriesType === 'bar'){
                                    res[index] = `${subitem.marker}${subitem.seriesName}: ${value}`
                                } else {
                                    res[index] += (` (▲${value})`)
                                }
                            })
