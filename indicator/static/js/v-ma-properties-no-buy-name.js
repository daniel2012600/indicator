...
限制:
範例:
    <v-ma-properties-no-buy-name :cdi='' @save=''></v-ma-properties-no-buy-name>
*/


Vue.component("v-ma-properties-no-buy-name", {
    delimiters: ['[[', ']]'],
    ...
    <el-form ref="no-buy-name" :model="form_data" label-position="top"　label-width="70px" :rules="rules" @submit.native.prevent>
        <el-form-item label="沒有購買產品名稱包含" prop="prd_name">
    ...
    data() {
        return {
            ...
            rules: {
                prd_name: [{ required: true, message: '字串不得為空'}]
            }
        };
    },
    watch: {
    ...
    methods:{
        is_valid(){
            //是否驗證通過
            var validated = false;
            if (this.$refs["no-buy-name"]) {
                this.$refs["no-buy-name"].validate((valid) => {
                    if (valid) {
                        validated = true;
                    }
                });
